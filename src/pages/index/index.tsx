...
class Index extends Component<PageProps, PageState> {
  ...
  navigateToUserCreate = () => {
    ...
    // If user has an ID, it's an update operation
    if (user && user.id) {
      console.log("Navigating to update user profile with ID:", user.id);
      this.navigateTo(`/pages/userCreate/index?id=${user.id}`);
    }
    // If user has openId but no id, it's a create operation
    else if (user && user.openId) {
      console.log(
        "Navigating to create user profile with openId:",
        user.openId
      );
      this.navigateTo(
        `/pages/userCreate/index?openId=${user.openId}&unionId=${
          user.unionId || ""
        }`
      );
    }
    // Fallback - just navigate to the page
      ...
      console.log("Navigating to user create page without parameters");
      this.navigateTo("/pages/userCreate/index");
    }
  };
  ...
  renderUserInfo = () => {
    ...
    // Default avatar URL when user is not fully created yet
    const defaultAvatar =
      "https://img2.baidu.com/it/u=1148608979,2798275823&fm=253&fmt=auto&app=138&f=JPEG?w=1145&h=800";

    // Check if this is a new user (has openId but no id/nickName)
    const isNewUser = user.openId && !user.nickName;

    return (
      <View className="user-info-display">
        <Image
          className="avatar"
          src={isNewUser ? defaultAvatar : user.avatarUrl || defaultAvatar}
          mode="aspectFit"
        />

        <View className="button-group">
          {isNewUser ? (
            // For new users, show only a Create User button
            <Button
            ...
            // For existing users, show the current buttons
            <>
              <Button
  ...
  renderPetsList = () => {
    ...
    // Don't show pets list for new users
    const isNewUser = user?.openId && !user?.nickName;
    if (isNewUser) {
      return (
        <View className="welcome-message">
          <Text className="welcome-title">欢迎来到 PlayTime!</Text>
          <Text className="welcome-subtitle">请先创建您的用户资料</Text>
        </View>
      );
    }
